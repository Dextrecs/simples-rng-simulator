...
const updateLog = [
    {
        ...
        features: [
            ...
            "Added a new 'Update Log' button to the main actions.",
            "Implemented dynamic content rendering for the Update Log."
        ],
        fixes: [
            "Fixed a minor display issue on the Omnipotent rarity badge."
        ]
    },
    {
        version: "1.0.0",
        date: "Dec 16, 2025",
        features: [
            "Initial game release: Core rolling and inventory system.",
            "Luck upgrade and Sell Commons function.",
            "Settings menu with theme, SFX volume, and particle control.",
            "Auto-Roll with speed upgrades."
        ]
    }
];
...
// NEW: Toggle Update Log Modal
function toggleUpdateLogModal() {
    document.getElementById('update-log-modal').classList.toggle('hidden');
}

// NEW: Render Update Log Content
function renderUpdateLog() {
    const container = document.getElementById('update-log-content');
    let html = '';
    
    updateLog.forEach(log => {
        // NOTE: Uses inline styles (var(--text), var(--primary), etc.) for theme compatibility
        html += `
            <div style="border: 1px solid var(--border); padding: 10px; margin-bottom: 15px; border-radius: 5px;">
                <h3 style="margin-top: 0; color: var(--primary);">v${log.version} - ${log.date}</h3>
                
                ${log.features && log.features.length > 0 ? `
                    <p style="font-weight: bold; margin-bottom: 5px;">New Features:</p>
                ...
                ${log.fixes && log.fixes.length > 0 ? `
                    <p style="font-weight: bold; margin-bottom: 5px;">Bug Fixes:</p>
                    <ul style="margin-left: 20px;">
                        ${log.fixes.map(f => `<li style="color: var(--danger);">${f}</li>`).join('')}
                    </ul>
                ` : ''}
            </div>
        `;
